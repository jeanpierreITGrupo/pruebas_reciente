...
class AccountBaseSunat(models.Model):
	...
	def _get_sql(self,type,period_id,company_id):
		...
		if type == 1:
			...
			END AS campo42,
			NULL AS campo43
			from get_compras_1_1('{date_start}','{date_end}',{company_id}) vst_c
			...
			END AS campo42,
			NULL AS campo43
			from get_compras_1_1('2000/01/01','{before_date}',{company_id}) vst_c
			left join account_move am on am.id = vst_c.am_id
		...
		if type in (4,5):
			...
			self.env.cr.execute("""select cuo from account_move_line where (cuo is null or cuo = 0) and (date between '%s' and '%s')""" % (period_id.date_start.strftime('%Y/%m/%d'),period_id.date_end.strftime('%Y/%m/%d')))
	...
	def pdf_get_sql_vst_12_cliente(self,date_start,date_end,company_id):
		sql = """
			...
			saldo_mn
			FROM get_saldos('{date_from}','{date_to}',{company_id},'date')
	...
	def pdf_get_sql_vst_41(self,date_start,date_end,company_id):
		sql = """
			...
			FROM get_saldos('{date_from}','{date_to}',{company_id},'date') gs
	...
	def get_sql_pdb_purchase(self,date_start,date_end,company_id):
		sql = """
				...
				LEFT JOIN get_compras_1_1('%s','%s',%d) vst_c ON vst_c.am_id = pdb.move_id
				LEFT JOIN account_move am ON am.id = vst_c.am_id
	...
	def get_sql_pdb_sale(self,date_start,date_end,company_id):
		sql = """
			...
			LEFT JOIN get_ventas_1_1('%s','%s',%d) vst_v ON vst_v.am_id = pdb.move_id
			LEFT JOIN account_move am ON am.id = vst_v.am_id
			LEFT JOIN res_partner rp ON rp.id = am.partner_id
			LEFT JOIN ( SELECT a2.type_document_id,
						a2.date,
	...
	def get_sql_pdb_payment(self,date_start,date_end,company_id):
		sql = """
				...
				FROM get_compras_1_1('%s','%s',%d) vst_c
