...
class FunctionResultWizard(models.TransientModel):
	...
	def get_function_totals(self,groups,totals):
		...
		operative_utility = get_sum_group('F1') + get_sum_group('F3')
	...
	def get_pdf_function_result(self):
		...
		return self.env['popup.it'].get_file('Resultado por Funcion.pdf',base64.encodestring(b''.join(f.readlines())))
	...
	def get_excel_function_result(self):
		...
		return self.env['popup.it'].get_file('Resultado por Funcion.xlsx',base64.encodestring(b''.join(f.readlines())))