...
class EfectiveFlowWizard(models.TransientModel):
	...
	def get_pdf_efective_flow(self):
		...
		return self.env['popup.it'].get_file('Flujo Efectivo.pdf',base64.encodestring(b''.join(f.readlines())))
	...
	def get_excel_efective_flow(self):
		...
		return self.env['popup.it'].get_file('Flujo Efectivo.xlsx',base64.encodestring(b''.join(f.readlines())))