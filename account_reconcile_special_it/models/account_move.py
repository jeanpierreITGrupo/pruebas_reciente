...
class AccountMove(models.Model):
	...
	def action_reconcile_special(self):
		for move in self:
			self._cr.execute("""update account_move set amount_residual = amount_total, amount_residual_signed = amount_total_signed, payment_state='not_paid' where id = %d"""%(move.id))
			self._cr.execute("""update account_move_line set reconciled=FALSE, amount_residual = (debit - credit) , amount_residual_currency = amount_currency where move_id = %d"""%(move.id))
