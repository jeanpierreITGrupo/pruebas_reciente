...
class ImportInvoiceIt(models.Model):
	...
	account_opt = fields.Selection([('default', 'Usar Cuenta Configurada en Producto'), ('custom', 'Usar Cuenta de Excel')], string='Cuenta de', required=True)
	type_import = fields.Selection([('out_invoice', 'Cliente'), ('in_invoice', 'Proveedor'),('out_refund',u'Nota Crédito Cliente'),('in_refund',u'Nota Crédito Proveedor')], string='Tipo', required=True, default='in')
			...
			if row_no <= 0:
				...
				if self.account_opt == 'default':
					if len(line) == 29:
						...
						raise UserError(_('Your File has extra column please refer sample file'))
					else:
						raise UserError(_('Your File has less column please refer sample file'))
				else:
					if len(line) == 30:
						if line[12] == '':
							raise UserError(_('Please assign a date'))
