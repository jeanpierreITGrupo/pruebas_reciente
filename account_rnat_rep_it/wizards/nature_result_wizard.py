...
class NatureResultWizard(models.TransientModel):
	...
	def get_pdf_nature_result(self):
		...
		return self.env['popup.it'].get_file('Resultado por Naturaleza.pdf',base64.encodestring(b''.join(f.readlines())))
	...
	def get_excel_nature_result(self):
		...
		return self.env['popup.it'].get_file('Resultado por Naturaleza.xlsx',base64.encodestring(b''.join(f.readlines())))