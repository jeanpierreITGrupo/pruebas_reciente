			...
			if row_no <= 0:
				...
				if self.import_product_type == 'create':
					...
					if line[7].strip()=='':
						...
						if ((line[7]).strip() == 'Cantidades entregadas'):
							politica_facturacion = 'delivery'
						elif ((line[7]).strip() == 'Cantidades pedidas'):
					...
					if line[11].strip()=='':
						...
						if ((line[11]).strip() == 'Sobre cantidades pedidas'):
							politica_control = 'purchase'
						elif ((line[11]).strip() == 'Sobre cantidades recibidas'):
