...
class AccountBookPerceptionWizard(models.TransientModel):
	...
	def get_report(self):
		if self.type == 'solo':
			self.env.cr.execute("""
			CREATE OR REPLACE view account_book_perception_sp as (select row_number() OVER () AS id, T.* FROM ("""+self._get_sql(self.period_id.date_start,self.period_id.date_end,self.company_id.id)+""")T )""")
		...
		if self.type == 'det':
			...
			CREATE OR REPLACE view account_book_perception
			 as (select row_number() OVER () AS id, T.* FROM ("""+self._get_sql(self.period_id.date_start,self.period_id.date_end,self.company_id.id)+""")T )""")
			if self.type_show == 'pantalla':
				return {
	...
	def get_excel(self):
		...
		self.env.cr.execute(self._get_sql(self.period_id.date_start,self.period_id.date_end,self.company_id.id))
