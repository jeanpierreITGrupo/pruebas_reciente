...
class Download_xls(http.Controller):
	...
	def download_template_import_initial(self,model,id, **kw):
		...
		if res.tipo == 'out':
			invoice_xls = request.env['ir.attachment'].search([('name','=','import_initial_customer.xlsx')])
			filecontent = invoice_xls.datas
			filename = 'Plantilla Saldos Iniciales por Cobrar.xlsx'
			filecontent = base64.b64decode(filecontent)
		
		else:
			invoice_xls = request.env['ir.attachment'].search([('name','=','import_initial_supplier.xlsx')])
			filecontent = invoice_xls.datas
