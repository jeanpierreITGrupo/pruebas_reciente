...
class ImportMoveAperturaIt(models.Model):
	...
	def importar(self):
		...
		self.env.cr.execute("""
			...
			left join l10n_latam_document_type ec on ec.code = iaa.n_tipo_doc
		...
		for acc in data:
			...
			if self.tipo == 'out':
				...
				vals = (0,0,{
					...
					'price_subtotal':acc[8] if currency.name != 'PEN' else acc[7],
					'price_total':acc[8] if currency.name != 'PEN' else acc[7],
					'price_unit':acc[8] if currency.name != 'PEN' else acc[7],
					'date_maturity':acc[2],
				...
				vals = (0,0,{
					...
					'price_subtotal':acc[8]*-1 if currency.name != 'PEN' else acc[7]*-1,
					'price_total':acc[8]*-1 if currency.name != 'PEN' else acc[7]*-1,
					'price_unit':acc[8]*-1 if currency.name != 'PEN' else acc[7]*-1,
					'date_maturity':acc[2],
					'company_id': self.company_id.id,
					'tc': acc[10] if currency.name != 'PEN' else 1,
