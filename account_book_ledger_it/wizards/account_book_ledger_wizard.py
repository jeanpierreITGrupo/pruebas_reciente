...
class AccountBookLedgerWizard(models.TransientModel):
	...
	def _get_sql(self):
		...
		sql = """SELECT
			may.periodo,may.fecha,may.libro,may.voucher,
	...
	def get_report(self):
		...
		if self.type_show == 'pantalla':
			self.env.cr.execute("""CREATE OR REPLACE view account_book_ledger_view as (SELECT row_number() OVER () AS id, T.* FROM ("""+self._get_sql()+""")T)""")
