...
class AccountExchangeDocumentRep(models.TransientModel):
	...
	def do_invoice(self):
		...
		if move_id_document:
			raise UserError('Ya existe un asiento de cambio generado para este periodo: '+move_id_document.name)
		...
		sql = """SELECT smd.*,ei.id AS type_document_id FROM get_saldos_me_documento_final('%s','%s',%d) smd
				 LEFT JOIN einvoice_catalog_01 ei ON ei.code = smd.td_sunat
				 WHERE smd.diferencia <> 0""" % (self.fiscal_year_id.name,self.period.code,self.company_id.id)
