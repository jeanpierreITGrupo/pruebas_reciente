...
class AccountBookSaleWizard(models.TransientModel):
	...
	def get_excel(self):
		...
		for line in res:
			...
			worksheet.write(x,c+24,line['currency_rate'] if line['currency_rate'] else '0.0000',formats['numbercuatro'])
			worksheet.write(x,c+25,line['fecha_det'] if line['fecha_det'] else '',formats['dateformat'])
			worksheet.write(x,c+26,line['comp_det'] if line['comp_det'] else '',formats['especial1'])
			worksheet.write(x,c+27,line['f_doc_m'] if line['f_doc_m'] else '',formats['dateformat'])
			worksheet.write(x,c+28,line['td_doc_m'] if line['td_doc_m'] else '',formats['especial1'])
			worksheet.write(x,c+29,line['serie_m'] if line['serie_m'] else '',formats['especial1'])
			worksheet.write(x,c+30,line['numero_m'] if line['numero_m'] else '',formats['especial1'])
			worksheet.write(x,c+31,line['glosa'] if line['glosa'] else '',formats['especial1'])
			x += 1
		...
		return self.env['popup.it'].get_file('RV%s.xlsx'%(self.company_id.partner_id.vat),base64.encodestring(b''.join(f.readlines())))
	...
	def _get_sql(self):
		if self.currency == 'pen':
			...
			sql = """SELECT periodo::character varying, fecha_cont, libro, voucher, fecha_e, fecha_v, td, 
					serie, anio, numero, tdp, docp, namep, exp, venta_g, inaf, exo, isc_v, icbper,
					otros_v, igv_v, total, name, currency_rate, fecha_det, 
					comp_det, f_doc_m, td_doc_m, serie_m, numero_m, glosa
					from get_ventas_1_1('%s','%s',%d,'%s')
				""" % (self.date_from.strftime('%Y/%m/%d'),
					self.date_to.strftime('%Y/%m/%d'),
