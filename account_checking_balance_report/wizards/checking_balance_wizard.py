...
class CheckingBalanceWizard(models.TransientModel):
	...
	def get_excel_checking_register(self):
		...
		widths = [8,9,40,10,10,12,12]
		worksheet = ReportBase.resize_cells(worksheet,widths)
		workbook.close()
		f = open(direccion +'Balance_Comprobacion.xlsx', 'rb')
		return self.env['popup.it'].get_file(u'Balance Comprobación - Registro.xlsx',base64.encodestring(b''.join(f.readlines())))
	...
	def get_excel_checking_balance(self):
		...
		widths = [9,40,10,10,12,12]
		worksheet = ReportBase.resize_cells(worksheet,widths)
		workbook.close()
		f = open(direccion +'Balance_Comprobacion.xlsx', 'rb')
		return self.env['popup.it'].get_file(u'Balance Comprobación - Balance.xlsx',base64.encodestring(b''.join(f.readlines())))